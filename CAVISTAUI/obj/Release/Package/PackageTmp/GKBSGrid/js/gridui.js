    ...
    this.columns.forEach(col => {
        ...
        //rowEl.appendChild(selectCell);
            ...
            this.columns.forEach(col => {
                ...
                cell.className = 'dg-cell';
                cell.style.flexBasis = (col.width || 150) + 'px';
                cell.style.minWidth = (col.width || 150) + 'px'; // Enforce minimum size
            cell.style.flexShrink = 0; // Ensure it never shrinks
...
// 1. RENDER SINGLE ICON ACTION (View)
    if (col.type === 'action') {
        ...
        const action = this.options.singleIconAction;
        
        if (action) { // Ensure the single action is defined
            ...
            actionBtn.onclick = (e) => {
                e.stopPropagation();
                // Pass rowData and grid instance
...
// 💡 NEW HELPER FUNCTION: Truncate/Round down to specified precision
    ...
    if (this.options.onCellChange && typeof this.options.onCellChange === 'function') {
        this.options.onCellChange(col.field, finalValue, rowData); 
            ...
            // IMPORTANT: Skip the generic 'change' listener below, as we handled update/recalc in 'onblur'
            return el; 
        } 
